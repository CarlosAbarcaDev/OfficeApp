...
interface Character {
    id: string;
    name: string;
    // Añade otros campos relevantes aquí.
  }
  ...
  export const useStore = create<FavoriteState>((set) => ({
    ...
    addFavorite: (character) => set((state) => {
      const newFavorites = state.favorites.some(fav => fav.id === character.id) 
                           ? state.favorites 
    ...
    removeFavorite: (characterId) => set((state) => {
      const newFavorites = state.favorites.filter(fav => fav.id !== characterId);
      localStorage.setItem('favorites', JSON.stringify(newFavorites));
      return { favorites: newFavorites };
    })
  }));